...
document.getElementById('sign-in').onclick = function(){
	...
	if($userMail === '' || $userPass === ''){
		...
		for(let user of users){
			if(user.email == $userMail && user.password == $userPass){
...
document.getElementById('make-account').onclick = function(){
...
document.getElementById('have-account').onclick = returnSignin;

document.getElementById('sign-up').onclick = function(){
	const $firstName = document.getElementById('FirstName').value;
	const $lastName = document.getElementById('LastName').value;
	const $makeEmail = document.getElementById('MakeEmail').value;
	const $makePassword = document.getElementById('MakePassword').value;
	if($firstName === '' || $lastName === '' || $makeEmail === '' || $makePassword === ''){
		alert('Hãy nhập đầy đủ thông tin');
	} else {
		let checkMail = 0;
		for(let user of users){
			if(user.email == $makeEmail){
...
function renderUser(data){
	...
	document.getElementById('tbody').innerHTML = htmlUser;
}


document.getElementById('searchButton').onclick = function(){
    const keyword = document.getElementById('for-search').value.toLowerCase();
    const result = [];
    for(let user of users){
        if(user.first_name.toLowerCase().includes(keyword) || user.last_name.toLowerCase().includes(keyword) || user.email.toLowerCase().includes(keyword)){
            result.push(user);
        }
    }
    renderUser(result);
	...
	for(let i=0; i< posts.length; i++){
		...
		for(let user of users){
			if(currentPost.user_id == user.id){
				peopleMake = user.first_name + user.last_name;
			}
		}
		htmlPost += `<tr>
						<th scope="row">${currentPost.id}</th>
						<td>${currentPost.title}</td>
						<td>${currentPost.created_at}</td>
						<td>${peopleMake}</td>
                  	</tr>
					`;
	}
	document.getElementById('pbody').innerHTML = htmlPost;
}


document.getElementById('searchPost').onclick = function(){
	const $postId = document.getElementById('post-search').value;
	for(let post of posts){
		...
		if(post.id == $postId){
			document.getElementById('post-detail').innerHTML = `
				<h4>Id: ${post.id}</h4>
                <h6>Tiêu đề: ${post.title}</h6>
                <p>Nội dung: ${post.content}</p>
                <a href="${post.image}">link ảnh: ${post.image}</a>
                <p>Người tạo: ${peopleMake}</p>
...
document.getElementById('posts-of-user').onclick = function(){
	const $userEmail = document.getElementById('post-by-user').value;
	let htmlIdPost = '';
	for(let user of users){
		if(user.email == $userEmail){
			for(let post of posts){
				if(post.user_id == user.id){
					htmlIdPost += `<li>${post.title}</li>`;
				}
			}
		}
	}
	document.getElementById('listPost').innerHTML = htmlIdPost;
}
...
document.getElementById('AllPosts').onclick = function(){
	...
	document.getElementById('all-post').style.display = 'block';
	document.getElementById('post-details').style.display = 'none';
	document.getElementById('post-user').style.display = 'none';
	renderPost();
}

document.getElementById('PostDetail').onclick = function(){
	document.getElementById('user-list').style.display = 'none';
	document.getElementById('all-post').style.display = 'none';
...
document.getElementById('PostByUser').onclick = function(){
	document.getElementById('user-list').style.display = 'none';
	document.getElementById('all-post').style.display = 'none';
	document.getElementById('post-details').style.display = 'none';
